...
def get_all_roles(role_service: RoleServices = Depends(role_services)):
    return role_service.get_all()

@router.post('/', response_model=RolePublicResponse)
def create_role(roleDTO: CreateRoleDTO, role_service: RoleServices = Depends(role_services)):
    return role_service.create(roleDTO)

@router.delete('/{role_id}', response_model=RolePublicResponse)
def delete_role(role_id: int, role_service: RoleServices = Depends(role_services)):
